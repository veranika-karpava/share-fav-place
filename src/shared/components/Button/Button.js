...
const Button = ({ href, size, inverse, danger, to, exact, type, onClick, disabled, children }) => {
    if (href) {
        return (
            <a
                className={`button__button-${size || 'default'} ${inverse &&
                    'button__button-inverse'} ${danger && 'button__button-danger'}`}
                href={href}
            >
                {children}
            </a>
    ...
    return (
        ...
        <button className={`button__button-${size || 'default'} ${inverse && 'button__button-inverse'} ${danger && 'button__button-danger'}`} type={type} onClick={onClick} disabled={disabled}>
